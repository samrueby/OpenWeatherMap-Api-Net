...
namespace OpenWeatherMap.Tests {
	...
	public class SearchClientTests: OpenWeatherMapTestsBase {
		...
		public async Task Can_Get_Search_By_City_Coordinates_MetricSystem() {
			var result = await OpenWeatherMapTestClient.Search.GetByCoordinates( new Coordinates { Latitude = Latitude, Longitude = Longitude }, MetricSystem.Metric );
			TestAllProperties( result );
			Assert.IsTrue( result.Mode == "center" );
			Assert.IsTrue( result.List.FirstOrDefault( x => x.City.Name == CityName ) != null );
			Assert.IsTrue( result.List.FirstOrDefault( x => x.Temperature.Unit == Metric ) != null );
		...
		public async Task Can_Get_Search_By_City_Coordinates_MetricSystem_Language() {
			var resultIt = await OpenWeatherMapTestClient.Search.GetByCoordinates( new Coordinates { Latitude = Latitude, Longitude = Longitude }, MetricSystem.Metric, OpenWeatherMapLanguage.IT );

			var resultFr = await OpenWeatherMapTestClient.Search.GetByCoordinates( new Coordinates { Latitude = Latitude, Longitude = Longitude }, MetricSystem.Metric, OpenWeatherMapLanguage.FR );
		...
		public async Task Can_Get_Search_By_Coordinates_Exception() {
			var result = await OpenWeatherMapTestClient.CurrentWeather.GetByCoordinates( new Coordinates { Latitude = -9999, Longitude = -9999 } );
		}

		[ TestMethod ]
		public async Task Can_Get_Search_Count() {
			var result = await OpenWeatherMapTestClient.Search.GetByCoordinates( new Coordinates() { Longitude = -2.15, Latitude = 57 }, MetricSystem.Internal, OpenWeatherMapLanguage.EN, 2 );
			TestAllProperties( result );
		...
		public async Task Can_Get_Search_Accurate() {
			var resultAccurate = await OpenWeatherMapTestClient.Search.GetByName( CityName, MetricSystem.Internal, OpenWeatherMapLanguage.EN, null, Accuracy.Accurate );

			var resultLike = await OpenWeatherMapTestClient.Search.GetByName( CityName, MetricSystem.Internal, OpenWeatherMapLanguage.EN, null, Accuracy.Like );
